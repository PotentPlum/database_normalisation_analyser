...
def _configure_test_source() -> None:
    ...
    host = "localhost"
    port = "1433"
    db_name = "OperationsDemo"
    driver = quote_plus(os.getenv("MSSQL_ODBC_DRIVER", "ODBC Driver 18 for SQL Server"))

    encoded_pw = quote_plus(password)
    test_url = (
        "mssql+pyodbc://sa:"
        f"{encoded_pw}"
        f"@{host}:{port}/{db_name}?driver={driver}&TrustServerCertificate=yes"
    )
