...
2. **Seed from the host** using the Python seeder; it targets `localhost:1433` and skips work if `OperationsDemo` already exists:
   ...
   # PowerShell
   python seed_operations_demo.py --password "${env:MSSQL_SA_PASSWORD}"  # or omit to fall back to the default
   ```
...
- Install a SQL Server ODBC driver (17 or 18). If the default `ODBC Driver 18 for SQL Server` name does not exist on your OS, set `MSSQL_ODBC_DRIVER` or pass `--driver` to the seeder.
- Ensure `pyodbc` is compiled against that driver. Driver download links: https://learn.microsoft.com/sql/connect/odbc/download-odbc-driver-for-sql-server.
- `sqlserver_3nf_audit.py test` and `seed_operations_demo.py` both honor `MSSQL_ODBC_DRIVER` for the driver name.
